...
__all__ = ['SPMTokenizer', 'create_spt_model']


class SPMTokenizer(TokenizerSpec):
    '''
    SPMTokenizer https://github.com/google/sentencepiece.
    '''
    ...
    def __init__(self, model_path: str, special_tokens: Optional[Union[Dict[str, str], List[str]]] = None):
        ...
        self.special_tokens={'<PAD>': -1, '<UNK>': 0, '<BOS>': 1, '<EOS>': 2} if special_tokens is None else special_tokens
        if isinstance(self.special_tokens, dict):
            for k,v in self.special_tokens.items():
                self.special_token_to_id[k]=v
